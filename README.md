# Istraživanje turističkog tržišta — AI asistent kolegija

Samostalni obrazovni AI asistent za kolegij **Istraživanje turističkog tržišta**
...
mediji, kviz, napredak i usmena vježba — sve visi o poglavlju. Odjeljci priručnika
(1.1, 4.4 …) nisu zasebne stranice: oni daju strukturu sažetku cjeline i
granularnost dohvatu, pa svaki citat zna iz kojeg je odjeljka i s koje stranice.
...
| Cjelina | Stanje |
| --- | --- |
| Struktura kolegija: 11 cjelina, 60 odjeljaka, stvarni brojevi stranica | ✅ `data/sadrzaj.json` |
| Ingest priručnika (DOCX → cjeline, sažetci, odjeljci, isječci, ugradnje) | ✅ `npm run ingest` |
...
| Chat u cjelini i opći chat, uz citate i procjenu pokrića | ✅ `/api/chat` |
...
| Kartice za učenje (pojam → definicija) po cjelini | ✅ (čeka odobrenje — vidi niže) |
| Kviz cjeline + završna provjera znanja | ✅ (čeka pitanja nastavnika — vidi niže) |
...
2. **Prag pokrića.** Ako najbolji dohvaćeni isječak ne prijeđe `RAG_MIN_SCORE`,
   ...
   `nedovoljno_konteksta` s prijedlogom odjeljaka. Jeftinije je i pouzdanije nego
...
npm run ingest                    # cjeline, sažetci, odjeljci, isječci → Supabase
...
Zaseban Vercel projekt, iste ENV varijable kao lokalno. Stranice cjelina su
statičke rute s revalidacijom svakih sat vremena — nakon novog ingesta izmjene se
na CDN-u vide s tim odmakom (ili odmah, uz ručni redeploy).
...
2. **Potvrda mapiranja:** `npm run struktura` izvuče cjeline/odjeljke i stvarne
   ...
   (spojiti odjeljke, preimenovati, promijeniti redoslijed) — ingest čita **nju**,
   pa se izvorne oznake iz dokumenta ne uzimaju zdravo za gotovo.
3. **Ingest:** `npm run ingest`. Sažetak cjeline je **doslovni tekst** cijelog
   poglavlja pretvoren u Markdown (odjeljci su `##`, pododjeljci `###`) — ništa se
   ne prepričava ni ne dodaje. Taj je sažetak podloga za kartice i kviz.
4. **Ciljevi, kartice i kviz:** pitanja isporučuje nastavnik (`npm run kviz:uvezi`).
   Skripta `npm run nacrti` može pripremiti **nacrte** iz sažetka cjeline, ali oni
...
5. **Mediji:** nastavnik ih učitava u Supabase Storage prema konvenciji
   `NN-kratki-opis.ext` i upisuje red u tablicu `mediji` (vezan uz **cjelinu**).
   Mediji nisu u gitu.
...
**Chat u cjelini** — dohvat unutar te cjeline, odgovori sažeti, uz citate stranica,
bez općeg disclaimera.

**Opći chat** — dohvat po cijelom priručniku, uz kratku napomenu
*„Odgovaram samo prema udžbeniku."*

**Usmena vježba** (`/usmena-vjezba` ili unutar cjeline) — trening za usmeni ispit,
**bez službenog ocjenjivanja**:

1. asistent postavi jedno pitanje iz odabrane cjeline — težište se svaki put
   nasumično veže uz drugi njezin odjeljak, pa uzastopne vježbe pokrivaju cijelu
   cjelinu (uz TTS čitanje naglas);
...
| `npm run ingest` | Puni ingest priručnika (`--suho`, `--poglavlje=N`) |
| `npm run ingest:dopunski` | Ingest dopunskih PDF izvora iz registra |
| `npm run nacrti -- --ciljevi --kartice --kviz` | Nacrti ciljeva, kartica i kviz pitanja (`odobreno = false`) |
...
- **Kvizovi i kartice su prazni dok nastavnik ne isporuči ili ne odobri sadržaj.**
  To je namjerno: pravilo „ne izmišljaj pitanja" ima prednost pred punim MVP-om.
  `npm run nacrti` pripremi prijedloge iz sažetka cjeline, ali oni ne izlaze pred
  studenta bez odobrenja.
- **Ciljevi učenja i kartice za učenje** priručnik ne sadrži kao zaseban tekst; i
  oni se pripremaju kao nacrti i prikazuju tek nakon odobrenja.
